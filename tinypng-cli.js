...
function checkProxy() {
  if (argv.p || argv.proxy) {
    ...
    request = request.defaults({ proxy: 'http://127.0.0.1:10809' })
  }
}
...
function checkHelp() {
  if (argv.h || argv.help) {
    console.log(
      ...
      "  -p, --proxy      Use proxy. eg: -p http://127.0.0.1:1080\n" +
      "  -h, --help       Show help"
    );
    return true
  }
              ...
              if (!error && response) {
                if (response.statusCode === 201) {
                  if (body.output.size < body.input.size) {
                    ...
                    totalSize += body.input.size - body.output.size
